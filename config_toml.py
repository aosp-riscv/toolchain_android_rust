...
def configure():
    ...
    with open(paths.rustc_path('config.toml'), 'w') as config_toml:
        ...
        cxx_linker_flags = ' -Wl,-rpath,' + ' -Wl,-rpath,'.join(paths.cxx_linker_path()) + \
        ' -L' + ' -L'.join(paths.cxx_linker_path()) + \
        ' -I' + ' -I'.join(paths.cxx_linker_path())
        ...
        def host_config(target):
            ...
            with open(wrapper_name, 'w') as f:
                f.write("""\
#!/bin/bash -v
{real_cc} -I{cxxstd} $* --target={target} {sysroot_flags} {cc_linker_flags}  -stdlib=libc++
