...
def run_failable_command(command: str, error_message: str, *args, **kwargs) -> subprocess.CompletedProcess:
...
def run_quiet_command(command: str, error_message: str, *args, **kwargs) -> int:
    """Runs a failable command with stdout and stderr directed to /dev/null"""
    return run_failable_command(command, error_message, *args, **(kwargs | SUBPROCESS_RUN_QUIET_DEFAULTS)).returncode
